...
// https://material.io/design/components/navigation-CustomDrawer.html#specs
...
///
/// Drawers are typically used with the [Scaffold.CustomDrawer] property. The child of
/// the CustomDrawer is usually a [ListView] whose first child is a [DrawerHeader]
/// that displays status information about the current user. The remaining
/// CustomDrawer children are often constructed with [ListTile]s, often concluding
/// with an [AboutListTile].
///
/// The [AppBar] automatically displays an appropriate [IconButton] to show the
/// [CustomDrawer] when a [CustomDrawer] is available in the [Scaffold]. The [Scaffold]
/// automatically handles the edge-swipe gesture to show the CustomDrawer.
///
/// {@animation 350 622 https://flutter.github.io/assets-for-api-docs/assets/material/CustomDrawer.mp4}
///
...
/// <https://m3.material.io/components/navigation-CustomDrawer/overview> for
/// more details. While the [CustomDrawer] widget can have only one child, the
...
///
/// ** See code in examples/api/lib/material/CustomDrawer/CustomDrawer.0.dart **
/// {@end-tool}
///
/// {@tool dartpad}
/// This example shows how to migrate the above [CustomDrawer] to a [NavigationDrawer].
///
/// ** See code in examples/api/lib/material/navigation_drawer/navigation_drawer.0.dart **
/// {@end-tool}
///
/// An open CustomDrawer may be closed with a swipe to close gesture, pressing the
/// escape key, by tapping the scrim, or by calling pop route function such as
/// [Navigator.pop]. For example a CustomDrawer item might close the CustomDrawer when tapped:
///
/// ```dart
/// ListTile(
...
///   onTap: () {
///     // change app state...
///     Navigator.pop(context); // close the CustomDrawer
///   },
/// );
/// ```
///
/// See also:
///
///  * [Scaffold.CustomDrawer], where one specifies a [CustomDrawer] so that it can be
///    shown.
///  * [Scaffold.of], to obtain the current [ScaffoldState], which manages the
///    display and animation of the CustomDrawer.
///  * [ScaffoldState.openDrawer], which displays its [CustomDrawer], if any.
///  * <https://material.io/design/components/navigation-CustomDrawer.html>
class CustomDrawer extends StatelessWidget {
  /// Creates a Material Design CustomDrawer.
  ///
  /// Typically used in the [Scaffold.CustomDrawer] property.
  ///
  ...
  /// Sets the color of the [Material] that holds all of the [CustomDrawer]'s
  /// contents.
  ///
  ...
  /// The z-coordinate at which to place this CustomDrawer relative to its parent.
  ///
  /// This controls the size of the shadow below the CustomDrawer.
  ///
  /// If this is null, then [DrawerThemeData.elevation] is used. If that
  /// is also null, then it defaults to 16.0.
  final double? elevation;

  /// The color used to paint a drop shadow under the CustomDrawer's [Material],
  /// which reflects the CustomDrawer's [elevation].
  ///
  ...
  /// The color used as a surface tint overlay on the CustomDrawer's background color,
  /// which reflects the CustomDrawer's [elevation].
  ///
  ...
  /// The semantic label of the CustomDrawer used by accessibility frameworks to
  /// announce screen transitions when the CustomDrawer is opened and closed.
  ///
  ...
  ///
  /// The [clipBehavior] argument specifies how to clip the CustomDrawer's [shape].
  ///
  /// If the CustomDrawer has a [shape], it defaults to [Clip.hardEdge]. Otherwise,
  ...
  Widget build(BuildContext context) {
    ...
    final bool isDrawerStart =
        CustomDrawerController.maybeOf(context)?.alignment !=
            DrawerAlignment.end;
    ...
    return Semantics(
      ...
      child: ConstrainedBox(
        ...
        child: Container(
          ...
          // child: Material(
          //   elevation:
          //       elevation ?? drawerTheme.elevation ?? defaults.elevation!,
          //   shadowColor:
...
///
/// Rarely used directly. CustomDrawer controllers are typically created automatically
/// by [Scaffold] widgets.
///
/// The CustomDrawer controller provides the ability to open and close a CustomDrawer, either
/// via an animation or via user interaction. When closed, the CustomDrawer collapses
/// to a translucent gesture detector that can be used to listen for edge
/// swipes.
///
/// See also:
///
///  * [CustomDrawer], a container with the default width of a CustomDrawer.
///  * [Scaffold.CustomDrawer], the [Scaffold] slot for showing a CustomDrawer.
class CustomDrawerController extends StatefulWidget {
  /// Creates a controller for a [CustomDrawer].
  ///
  /// Rarely used directly.
  ///
  /// The [child] argument is typically a [CustomDrawer].
  ...
  /// close the CustomDrawer.
  final DrawerAlignment alignment;

  /// Optional callback that is called when a [CustomDrawer] is opened or closed.
  final DrawerCallback? drawerCallback;

  /// {@template flutter.material.CustomDrawerController.dragStartBehavior}
  /// Determines the way that drag start behavior is handled.
  ///
  /// If set to [DragStartBehavior.start], the drag behavior used for opening
  /// and closing a CustomDrawer will begin at the position where the drag gesture won
  ...
  /// Whether or not the CustomDrawer is opened or closed.
  ///
  /// This parameter is primarily used by the state restoration framework
  /// to restore the CustomDrawer's animation controller to the open or closed state
  /// depending on what was last saved to the target platform before the
  /// application was killed.
  final bool isDrawerOpen;

  /// The closest instance of [CustomDrawerController] that encloses the given
  /// context, or null if none is found.
  ///
  /// {@tool snippet} Typical usage is as follows:
  ///
  /// ```dart
  /// CustomDrawerController? controller = CustomDrawerController.maybeOf(context);
  /// ```
  /// {@end-tool}
  ///
  /// Calling this method will create a dependency on the closest
  /// [CustomDrawerController] in the [context], if there is one.
  ///
  /// See also:
  ///
  /// * [CustomDrawerController.of], which is similar to this method, but asserts
  ///   if no [CustomDrawerController] ancestor is found.
  static CustomDrawerController? maybeOf(BuildContext context) {
    return context
  ...
  ///
  /// Calling this method will create a dependency on the closest
  /// [CustomDrawerController] in the [context].
  ///
  /// {@tool snippet} Typical usage is as follows:
  ///
  /// ```dart
  /// CustomDrawerController controller = CustomDrawerController.of(context);
  /// ```
  /// {@end-tool}
  static CustomDrawerController of(BuildContext context) {
    final CustomDrawerController? controller = maybeOf(context);
    assert(() {
      if (controller == null) {
        throw FlutterError(
          'CustomDrawerController.of() was called with a context that does not '
          'contain a CustomDrawerController widget.\n'
          'No CustomDrawerController widget ancestor could be found starting from '
          'the context that was passed to CustomDrawerController.of(). This can '
          'happen because you are using a widget that looks for a CustomDrawerController '
...
///
/// Typically used by a [Scaffold] to [open] and [close] the CustomDrawer.
class DrawerControllerState extends State<CustomDrawerController>
  ...
  void didUpdateWidget(CustomDrawerController oldWidget) {
    ...
    if (_controller.status.isAnimating) {
      return; // Don't snap the CustomDrawer open or shut while the user is dragging.
    }
  ...
  double get _width {
    ...
    // return _kWidth if CustomDrawer not being shown currently
  ...
  /// Starts an animation to open the CustomDrawer.
  ///
  ...
  /// Starts an animation to close the CustomDrawer.
  void close() {
...
// BEGIN GENERATED TOKEN PROPERTIES - CustomDrawer
...
// END GENERATED TOKEN PROPERTIES - CustomDrawer
